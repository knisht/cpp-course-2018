...
big_integer::big_integer(std::string const& str)
{
    ...
    for (size_t i=str.size()-1; i<str.size(); i--)
    {
        ...
        *this += multiply_digit(multiplier, (ui)(str[i]-'0'));
...
void big_integer::add_absolute(big_integer& a, big_integer const& rhs)
{
    ...
    {
        ull test = (ull)a.number[i]+(ull)rhs.number[i]+carry;
        a.number[i]+=rhs.number[i]+carry;
        (test>logic_base) ? carry = 1 : carry = 0;
    }
    for (size_t i = std::min(a.number.size(), rhs.number.size()); i < std::max(a.number.size(), rhs.number.size()); i++)
    {
        if (a.number.size()<rhs.number.size())
            a.number.push_back(rhs.number[i]);
        ull test = (ull)a.number[i]+(ull)carry;
        a.number[i]+=carry;
        (test>logic_base) ? carry = 1 : carry = 0;
    }
    if (carry)
...
void big_integer::sub_absolute(big_integer& a, big_integer const& rhs)
{
    ...
    while (carry) {
        if (a.number.size()<rhslen) a.number.push_back(0);
        ui digit;
...
void big_integer::change_sign(big_integer& a)
{
    a.sign*=-1;
}
...
big_integer& big_integer::operator&=(big_integer const& rhs)
{
    *this = this->logic_operation(*this, rhs, [&](ui a, ui b) {
        return a&b;
    });
    return *this;
}
...
{
    ui delta = 0;
    std::vector<ui> result(number.size() + (rhs+31)/32);
    size_t j = 0;
    ...
    for (size_t i = 0; i < number.size(); i++) {
        ...
        result[j++] = newdigit;
        delta = (ui)((ull)(number[i]>>(bits_in_base-rhs)));
    }
    if (delta)
        result[j++] = delta;
    number = result;
...
big_integer& big_integer::operator>>=(int rhs)
{
    ...
    for (size_t i = number.size()-1; (i>=start)&&(i<number.size()); i--) {
        ui newdigit = (number[i]>>rhs)&((ui)((ui)1<<((ui)bits_in_base-(ui)rhs))-1);
        newdigit |= delta;
        delta = (number[i]&((ui)((ui)((ui)1<<(ui)rhs)-1)))<<(ui)(bits_in_base-rhs);
        result.push_back(newdigit);
    }
...
bool operator<(big_integer const& a, big_integer const& b)
{
    ...
    if (b.number.size()>a.number.size()) return true;
...
void big_integer::multiply_absolute(big_integer& a, big_integer const& b)
{
    ...
    for (size_t i = 0; i < a.number.size(); i++) {
        ...
        for (size_t j = 0; j < b.number.size(); j++) {
            storage = (ull)a.number[i]*(ull)b.number[j];
            tmp = (storage&logic_base) + result.number[i+j] + borrow;
            result.number[i+j] = (ui)(tmp&logic_base);
            borrow = (storage>>(bits_in_base)) + (tmp>>(bits_in_base));
        }
        result.number[i+b.number.size()] += (ui)borrow;
    }
    normalize(result);
    std::swap(a,result);
}
...
big_integer big_integer::multiply_digit(big_integer const& num, ui x)
{
    ...
    {
        ull product = tmp.number[i]*(ull)(x) + remainder;
        tmp.number[i] = (ui)(product & (logic_base));
        remainder = product>>(bits_in_base);
    }
    if (remainder) tmp.number.push_back(remainder);
    tmp.sign = 1;
    normalize(tmp);
    return tmp;
}
...
ui big_integer::trial(big_integer const& r, big_integer const& d, ui k, ui m)
{
    ui km = k+m;
    if (r==0) return 0;
    ull r1 = ((ull)r.number[km]<<(bits_in_base))+((ull)r.number[km-1]);
    ull d1 = (ull)d.number[m-1];
    ull left = r1/d1;
    return std::min((ui)(left), logic_base);
}

bool big_integer::cmp_prefix(big_integer const& r, big_integer const& d, ui k, ui m)
{
    size_t i = m, j = 0;
    while (i!=j) {
...
std::pair<big_integer,big_integer> big_integer::divide(big_integer const& x, big_integer const& y)
{
    big_integer r, q;
    q.sign = 1;
    size_t n = x.number.size(), m = y.number.size();
    ull e = base;
    ui f = (ui)(e / (y.number[y.number.size()-1]+1));
    if (f!=0) r = multiply_digit(x, f);
    else r = x<<(bits_in_base);
    big_integer d = multiply_digit(y, f);
    if (f==0) d = y<<(bits_in_base);
    ...
    if (f!=0) r = digit_quot(r,f);
    else r = r>>(bits_in_base);
    normalize(q);
...
std::pair<big_integer, big_integer> big_integer::division(big_integer const& a, big_integer const& b)
{
    ...
    if (a<b) return std::pair<big_integer, big_integer> {0, a};
    if (a==b) return std::pair<big_integer, big_integer> {1, 0};
    if (b.number.size() == 1) {
        return std::pair<big_integer, big_integer> {digit_quot(a,b.number[0]), digit_rem(a,b.number[0])};
    }
    return divide(a, b);
}
...
{
    while ((a.number.back()==0)&&(a.number.size()>1)) a.number.pop_back();
    if (a.number.size()==1&&a.number[0]==0) a.sign = 0;
}
...
{
    if ((a.number.size()==1)&&(a.number[0])==0) return true;
    else return false;
}
